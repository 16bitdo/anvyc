...
| **CP-5** creds | `anvyc creds {status\|rotate}` + `creds-expiry-within-7d` check | AWS SSO / GitHub PAT / Claude OAuth 만료 사전 감지 + native re-auth 위임 | [design-axes/cp-05-creds.md](./design-axes/cp-05-creds.md) |
