...
stdin 으로 9 도구 prompt + 마지막 confirm 에 답한다.
typer 의 default 처리:
...
def test_wizard_all_default_writes_yaml(tmp_path: Path) -> None:
    ...
    # 9 enable confirm + 5 file prompt (shell/git/aws/gh/pulumi) +
    # 2 dev_env prompt (roots, patterns) + 1 final write confirm = 17 Enter
    proc = _anvyc(
        "init", "--interactive",
        "--root", str(tmp_path),
        input_str=_enter_only(17),
        cwd=tmp_path,
    )
    ...
    # 9 tools 모두 enabled (dev_env 만 default n 이지만 wizard prompt 가 default y 로 안 함)
    # → 8 tools enabled (dev_env default disabled)
