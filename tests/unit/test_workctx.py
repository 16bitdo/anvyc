...
def test_is_expired_for_explicit() -> None:
...
def test_activity_row_never_expired() -> None:
...
def test_switch_writes_explicit_row(tmp_path: Path) -> None:
...
def test_switch_preserves_activity_rows(tmp_path: Path) -> None:
...
def test_switch_cleans_expired_explicit(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
...
def test_clear_removes_explicit_only(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
    cache.write_text(
        "500\tcwd_changed\t/x\told=/y\n"
...
def test_clear_on_missing_cache(tmp_path: Path) -> None:
...
def test_status_priority_explicit_over_activity(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
...
def test_status_expired_explicit_falls_back_to_activity(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
    cache.write_text(
...
def test_status_stale_activity(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
...
def test_status_empty_cache(tmp_path: Path) -> None:
    cache = tmp_path / "missing"
...
def test_write_cache_fifo_truncation(tmp_path: Path) -> None:
    cache = tmp_path / "cache"
    rows = [
...
def test_resolve_cache_path_env_wins(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_cleanup_expired_explicit_filters_correctly() -> None:
    rows = [
