...
- **진행 중 작업**: 없음 — v0.12.0 릴리스 완결. `git tag v0.12.0` → `release.yml` 가 GitHub Release 게시, in-repo + tap(`16bitdo/homebrew-anvyc`) Homebrew formula 를 v0.12.0 으로 갱신(Phase C). ⚠️ anvyc repo 가 private 이라 `brew install` 등 익명 설치 경로는 비동작 — public 전환 또는 PyPI 배포 결정 시 동작 (위 §2 2026-05-22 결정 참조).
- **다음 후보 (우선순위 낮음)**: 없음 — 소진된 구 개선 계획 문서 3종(ai-agent·ux-review·account-routing)은 `docs/archive/` 로 이동 완료 (2026-05-22).
