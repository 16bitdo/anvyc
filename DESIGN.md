...
### 33.3 project doctor check 명세 (14 check)
...
| `worktree_rule_links` | linked worktree 일 때만 (`.git` 이 파일) — `.cursor/rules`·`.cursor/skills`·`CLAUDE.md` symlink 연결 여부. `anvyc worktree add` 는 자동 연결하므로 직접 `git worktree add` 를 쓴 경우를 잡는다 | WARNING |
...
| `public_repo_email_exposure` | 선언 commit_email 이 noreply 가 아니고 저장소가 PUBLIC 이며 계정이 이메일을 비공개로 둘 때만 (판정 근거를 이름 규칙이 아니라 계정 설정에 둔다. 조회 실패는 silent) | WARNING |
