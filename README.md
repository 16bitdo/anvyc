...
- **(계획) CP-14 Secret Broker** — 사용 편의성↑ + "anvyc 평문 미접촉" 불변식 유지. 값 custody 는 op/sops/keychain/aws-vault 에 두고 anvyc 는 reference 레지스트리 + 와이어링 + 검증만 담당 (`anvyc secret {list|add|get|inject-wire}` + `secret-registry-valid` doctor check). Phase 1·2 는 불변식 무수정, passthrough(Phase 3)는 거버넌스 PR 선행 ([설계](./docs/design-axes/cp-14-secret-broker.md))
