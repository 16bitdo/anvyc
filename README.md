...
- **CP-15 Secret Broker** ✓ (Phase 1·2·2.5, anvyc#111~#117) — secret 입력/조회 broker. "anvyc 평문 미접촉" 불변식 유지: 값 custody 는 op/sops/keychain/aws-vault 에 두고 anvyc 는 reference 레지스트리 + 검증 + JIT 와이어링만 담당 (`anvyc secret {list|add|get|inject-wire}` + `secret-registry-valid` doctor check). passthrough(Phase 3 — anvyc 가 기존 값 직접 hidden-input)는 rule 26 / SECURITY 재정의 거버넌스 PR 선행으로 추후 ([설계](./docs/design-axes/cp-15-secret-broker.md))
