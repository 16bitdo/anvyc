...
<!-- BEGIN supported-tools — generated by scripts/gen_supported_tools.py (do not edit by hand) -->
| 도구 | 분류 | 기본 포함 | 기본 제외 | 도입 |
|---|---|---|---|---|
| Shell (zsh) | shell | `~/.zshrc`, `~/.zprofile`, `~/.zshenv`, `~/.zlogin` | `~/.zsh_history`, `~/.zsh_sessions`, `~/.zcompdump*` | v0.1.0 |
| Git | vcs | `~/.gitconfig`, `~/.gitignore_global` | `~/.git-credentials`, `~/.ssh/id_rsa`, `~/.ssh/id_ed25519` | v0.1.0 |
| AWS CLI | cloud | `~/.aws/config` | `~/.aws/credentials`, `~/.aws/sso/cache` | v0.1.0 |
| GitHub CLI | vcs | `~/.config/gh/config.yml` | `~/.config/gh/hosts.yml` | v0.1.0 |
| Cursor IDE | ide | `rules`, `skills`, `mcp.json`, `plugins`, `settings.json`, `keybindings.json`, `snippets` | `workspaceStorage`, `globalStorage`, `History`, `extensions`, `projects`, `chats` | v0.1.0 |
| Claude Code | ai-agent | `settings.json`, `keybindings.json`, `CLAUDE.md`, `file-suggestion.sh`, `.env.template`, `.statusline.config`, `hooks`, `plugins`, `plans` | `sessions`, `tokens`, `cache`, `logs`, `projects`, `config.json` | v0.1.0 |
| iTerm2 | terminal | `profiles`, `key mappings`, `color presets` | `window state`, `recent sessions`, `window arrangements` | v0.1.0 |
| Pulumi | iac | `~/.pulumi/config.json` | `~/.pulumi/credentials.json`, `~/.pulumi/access_tokens` | v0.1.0 |
| dev_env | dev-env | `.envrc`, `.tool-versions`, `.python-version`, `.nvmrc` | `**/node_modules/**`, `**/.venv/**`, `**/venv/**`, `**/.git/**`, `**/__pycache__/**` | v0.7.0 |
| Shell Prompt | shell | `~/.config/starship.toml`, `~/.p10k.zsh` | — | v0.13.0 |
<!-- END supported-tools -->

> 위 표는 [`scripts/gen_supported_tools.py`](./scripts/gen_supported_tools.py) 가 각 adapter 의
> `AdapterMeta` (단일 SoT) 에서 생성한다. 도구를 추가/변경하면 이 스크립트를 재실행해 동기화한다
> (CI 의 `--check` 가 drift 를 차단). 도구별 enabled/detect 현황은 `anvyc tools list`.

### 4.1 동반 도구 (companion tools — 선택)

위 §4 가 anvyc 가 *관리(백업)* 하는 도구라면, 아래는 anvyc 의 일부 기능을 *잠금 해제* 하는
**외부 CLI 바이너리 + Python extras** 다. 코어 동작에는 필요 없으며, 쓰려는 기능에 맞춰
필요한 것만 설치하면 된다. 현재 머신의 설치 상태는 **`anvyc extras`** 로 확인한다
(`--missing` 미설치만, `--json` 기계 가독, `--check` 필수 누락 시 exit 1).

<!-- BEGIN companion-tools — generated by scripts/gen_extras.py (do not edit by hand) -->
| 도구 | 종류 | 잠금 해제 기능 | 설치 |
|---|---|---|---|
| SOPS | CLI | secret_files 암호화/복호화 (encryption-at-rest) | `brew install sops` |
| age | CLI | SOPS age backend (키 생성·암복호화) | `brew install age` |
| 1Password CLI | CLI | op:// Secret Reference 검증·주입 | `brew install 1password-cli` |
| aws-vault | CLI | aws-vault secret backend | `brew install aws-vault` |
| GitHub CLI | CLI | GitHub cost 수집 (gh auth token) | `brew install gh` |
...
```bash
# uv tool (권장)
uv tool install https://github.com/16bitdo/anvyc/releases/download/v0.20.0/anvyc-0.20.0-py3-none-any.whl

# 또는 pipx
pipx install https://github.com/16bitdo/anvyc/releases/download/v0.20.0/anvyc-0.20.0-py3-none-any.whl
...
anvyc doctor                   # 환경 진단 (28 check, claude doctor 스타일 출력)
...
anvyc config projects {list|add|rm|exclude|unexclude}   # 개별 프로젝트 포함/제외 (anvyc.yaml projects/exclude_projects)
